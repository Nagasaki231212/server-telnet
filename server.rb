...
loop do
  ...
  Thread.new(cliente_en_puerta) do |cliente_privado|
    cliente_privado.puts 'Servidor Restringido, Ingresa la contraseña de acceso Manco:'

    intento_clave = cliente_privado.gets

    if intento_clave && intento_clave.chomp.strip == 'QuieroPito'

      cliente_privado.puts 'Bienvenido al servidor Telnet de Nagasaki.'
      cliente_privado.puts "Escribe un comando ('time', 'weather', 'get [archivo]' o 'quit', 'exit') :"
      ...
      cliente_privado.puts 'Contraseña incorrecta'
    end
